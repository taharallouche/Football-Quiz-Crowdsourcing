...
def prepare_data(
    path: str = "data/data_quiz_foot.csv",
    ...
    Answers.drop(
        Answers[(Answers.Voter == "Correction987")].index, inplace=True
    )  # Remove the first row, contains the ground truth
    ...
    for voter in Answers["Voter"]:
        for image in Images:
            ...
            tmp_row = {
                "Voter": voter,
                "Images": image,
                "RealMadrid": int("RealMadrid" in L),
                "Barcelone": int("Barcelone" in L),
                "BayernMunich": int("BayernMunich" in L),
                "InterMilan": int("InterMilan" in L),
                "PSG": int("PSG" in L),
            }
