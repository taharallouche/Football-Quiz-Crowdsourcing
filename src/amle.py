...
def amle_free(
    ...
    while (iteration == 0) or (
        (iteration <= iter_max)
        and (
            (np.max(abs(pq[["p", "q"]].to_numpy() - pq_1[["p", "q"]].to_numpy())) > eps)
            or (np.max(abs(t[["t"]].to_numpy() - t_1[["t"]].to_numpy())) > eps)
        )
    ):
...
def update_agg_free(
    Annotations: pd.DataFrame, agg: pd.DataFrame, pq: pd.DataFrame, t: pd.DataFrame
) -> None:
    """
    ...
    tau = sum(
        [
            np.log(
                (1 - pq.loc[pq.Voter == voter, "q"].values[0])
                / (1 - pq.loc[pq.Voter == voter, "p"].values[0])
            )
            for voter in list(Annotations.Voter.unique())
        ]
    )
    ...
    for image in agg.Images:
        for team in t.Team:
            ...
            approval.loc[approval.Team == team, "app_score"] = np.log(
                t.loc[t.Team == team, "t"].values[0]
                / (1 - t.loc[t.Team == team, "t"].values[0])
            )

            # Sum of the weights of the voters who approved the alternative
            approval.loc[approval.Team == team, "app_score"] += sum(
                [
                    np.log(
                        pq.loc[pq.Voter == voter, "p"].values[0]
                        * (1 - pq.loc[pq.Voter == voter, "q"].values[0])
                    ...
                    for voter in list(Annotations.Voter.unique())
                    if Annotations.loc[
                        (Annotations.Images == image) & (Annotations.Voter == voter),
                        team,
                    ].values[0]
                    == 1
                ]
...
def update_prior_free(agg: pd.DataFrame, t: pd.DataFrame) -> None:
    """
...
def update_pq(Annotations: pd.DataFrame, agg: pd.DataFrame, pq: pd.DataFrame) -> None:
    """
    ...
    for voter in Annotations.Voter.unique():
        ...
        # To avoid division by 0 and log(0)
        pq.loc[pq.Voter == voter, "p"] = max([min([tp / total_pos, 0.9999]), 0.0001])
        pq.loc[pq.Voter == voter, "q"] = max([min([fp / total_neg, 0.9999]), 0.0001])


# Update the prior parameters
def update_prior(agg: pd.DataFrame, t: pd.DataFrame) -> None:
    """
    ...
    for team in Teams:
        ...
        alpha_in = np.prod(
            np.array([1 - t[t.Team == h]["t"].values[0] for h in Teams if h != team])
        )
        alpha_in += sum(
            [
                t[t.Team == h]["t"].values[0]
                * np.prod(
                    np.array(
                        [
                            1 - t[t.Team == k]["t"].values[0]
                            for k in Teams
                            if k not in [team, h]
                        ]
                    )
        ...
        alpha_out += sum(
            [
                t[t.Team == h]["t"].values[0]
                * t[t.Team == k]["t"].values[0]
                * np.prod(
                    np.array(
                        [
                            1 - t[t.Team == l]["t"].values[0]
                            for l in Teams
                            if l not in [team, h, k]
                        ]
                    )
                )
                for h in Teams
...
def update_agg(
    ...
    for image in agg.Images:
        ...
        if (
            sum(
                [
                    int(approval[approval.Team == team]["app_score"].values[0] >= tau)
                    for team in approval.Team
                ]
            )
            <= 1
        ):
            ...
            top_team = approval.loc[approval["app_score"].idxmax(), "Team"]
            for team in t.Team:
                if team == top_team:
                    agg.loc[agg.Images == image, team] = 1
                else:
            ...
            top_team_2 = approval[approval.Team != top_team].loc[
                approval[approval.Team != top_team]["app_score"].idxmax(), "Team"
            ]
            for team in t.Team:
...
def initialize_pq(Annotations: pd.DataFrame, pq: pd.DataFrame) -> None:
    """
    ...
    for voter in V:
        ...
        t = sum(
            [
                distance.jaccard(
                    Annotations.loc[Annotations.Voter == voter, Teams]
                    .to_numpy()
                    .astype(int)
                    .flatten(),
                    Annotations.loc[Annotations.Voter == v, Teams]
                    .to_numpy()
                    .astype(int)
                    .flatten(),
                )
    ...
    for voter in V:
        ...
        w = (c / (1 + c) - 1 / (1 + c)) * (
            1 / d.loc[d.Voter == voter, "d"].values[0] - mini
        ) / (maxi - mini) + 1 / (1 + c)
